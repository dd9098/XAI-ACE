...
class ConceptDiscovery(object):
            ...
            for s in range(segments.max()):
                ...
                if np.mean(mask) > 0.001:
                    ...
                    for seen_mask in unique_masks:
                        jaccard = np.sum(seen_mask * mask) / \
                            np.sum((seen_mask + mask) > 0)
                        if jaccard > 0.5:
                            unique = False
                            break
                    if unique:
    ...
    def _extract_patch(self, image, mask):
        ...
        h1, h2, w1, w2 = ones[0].min(), ones[0].max(
        ), ones[1].min(), ones[1].max()
    ...
    def _random_concept_activations(self, bottleneck, random_concept):
        ...
        Args:
          ...
          random_concept: Name of the random concept e.g. "random300_0"
    ...
    def _concept_cavs(self, bn, concept, activations, randoms=None, ow=True):
        ...
        if randoms is None:
            randoms = [
                'random300_{}'.format(i) for i in np.arange(self.num_random_exp)
            ]
        if self.num_workers:
            pool = multiprocessing.Pool(20)
            accs = pool.map(
                lambda rnd: self._calculate_cav(
                    concept, rnd, bn, activations, ow),
                randoms)
        else:
            accs = []
            for rnd in randoms:
                accs.append(self._calculate_cav(
                    concept, rnd, bn, activations, ow))
        return accs
    ...
    def cavs(self, min_acc=0., ow=True):
        ...
        for bn in self.bottlenecks:
            for concept in self.dic[bn]['concepts']:
                ...
                acc[bn][concept] = self._concept_cavs(
                    bn, concept, concept_acts, ow=ow)
            ...
            rnd_acts = self._random_concept_activations(
    ...
    def tcavs(self, test=False, sort=True, tcav_score_images=None):
        ...
        randoms = ['random300_{}'.format(i)
                   for i in np.arange(self.num_random_exp)]
        if tcav_score_images is None:  # Load target class images if not given
            raw_imgs = self.load_concept_imgs(
                self.target_class, 2 * self.max_imgs)
    ...
    def delete_concept(self, bn, concept):
        ...
        if concept in self.dic[bn]['concepts']:
            self.dic[bn]['concepts'].pop(
                self.dic[bn]['concepts'].index(concept))
    ...
    def find_profile(self, bn, images, mean=True):
        ...
        for i, concept in enumerate(self.dic[bn]['concepts']):
            profile[:, i, :] = self._concept_profile(
                bn, class_acts, concept, randoms)
        if mean:
